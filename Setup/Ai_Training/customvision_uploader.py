...
prediction_credentials = ApiKeyCredentials(in_headers={"Prediction-key": conf.PREDICTION_KEY})
predictor = CustomVisionPredictionClient(conf.PREDICTION_ENDPOINT, prediction_credentials)
...
def tag_species(species):
    ...
    for file_name in file_list:
        with open(os.path.join (base_image_location, species,
            file_name), "rb") as tag_image_contents:
            image_list.append(ImageFileCreateEntry(name=file_name,
                contents=tag_image_contents.read(), tag_ids=[tag.id]))
...
trainer.publish_iteration(project.id, iteration.id,
    PUBLISH_ITERATION_NAME, conf.PREDICTION_RESOURCE_ID)
print ("Done!")

prediction_credentials = ApiKeyCredentials(in_headers={"Prediction-key": conf.PREDICTION_KEY})
predictor = CustomVisionPredictionClient(conf.PREDICTION_ENDPOINT, prediction_credentials)

with open(os.path.join (conf.DIR_PATH, "Test/test_image.jpeg"), "rb") as test_image_contents:
